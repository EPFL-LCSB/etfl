...
from ..optim.constraints import EnzymeDeltaPos, EnzymeDeltaNeg, \
    mRNADeltaPos, mRNADeltaNeg, ModelConstraint
...
def compute_center_generic(dmodel, provided_solution=None):
    """
    Fixes the objective to be above computed lower bound, finds chebyshev center,
    resets the model, returns solution data. It laso fixes the growth at its previous value
    ...
    # to fix the objective
    tol = 0.01
    prev_obj = dmodel.objective.expression
    obj_val = dmodel.objective.value
    
    fix_growth(dmodel, the_solution)
    
    fix_obj = dmodel.add_constraint(kind = ModelConstraint, 
                             hook = dmodel, 
                             expr = prev_obj,
                             id_ = 'fix_objective',
                             lb = obj_val - abs(tol * obj_val),
                             ub = obj_val + abs(tol * obj_val),)
    ...
    dmodel.remove_constraint(fix_obj)
    dmodel.growth_reaction.lower_bound = prev_lb
    dmodel.objective = prev_obj
    return chebyshev_sol